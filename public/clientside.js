...
function displayItems()
{
    $.get("/getItems", function(data) {
        ...
        for (var i = 0; i < data.list.length; i++) {
            ...
            if(i == 0){
                itemID = item.item_id;
                $("#displayItems_id").append("<tr onclick=\"checkInfo(" + i + ")\"> <td>" + item.ItemName + "<td><img src=\"" + item.PicPath + "\" > <td>" + item.Description + "<td>" + item.ItemPrice + "<td><input type=\"checkbox\" name=\"cart" + i + "\" id=\"" + i + "\" value=\"1\"");
            }
            else{
                itemID = item.item_id;
